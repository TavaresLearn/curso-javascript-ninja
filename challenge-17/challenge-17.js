(function() {
  ...
  */
  ...
  console.log( text.replace( /Manuel Marques de Sousa/g, 'Diego Tavares') );
  ...
  */
  ...
  console.log( text.replace( /a/gi, 4) ); // adicionado o 'i' que ignora maiúsculas e minúsculas
  ...
  */
  ...
  // let regexDate = /(13|18) de (junho|julho) de (1804|1875)/;
  ...
  */
  console.log( '\nReplace de datas:' );
  function replaceDate(_regex, day, month, year) {
    return `${day}/${getMonthNumber(month)}/${year}`;
  }
  console.log( text.replace(regexDate, replaceDate));

  //
  // explicação de porque não foi utilizado as funções regexDate() e replaceDate() com os ()
  //
  /* O replace espera que uma função seja passada como segundo argumento. Se você executar a função, o que vai
  acontecer é: quando essa linha for interpretada, a função vai ser executada, e o seu retorno é que vai ser passado
  para o replace, e não é isso que a gente quer :)

  Veja que esse código é equivalente a:
  text.replace(regexDate, function () {
  // aqui o código da função
  })
  Se você reparar, no código acima nós CRIAMOS uma função, já passando para o replace. Mas nós não executamos a função :)
  Já no exemplo da aula, a única coisa que fizemos diferente disso foi criar uma função literal, com o nome replaceDate,
  e passamos a referência da função sem executa-la.

  Quem vai fazer a execução da função é o replace :) */
})();
